...
test("manifest and catalog are valid", async () => {
  ...
  assert.equal(catalog.metas[0].id, "cartoon-aziz:sally-2026");
...
test("Sally has 48 episodes and an encoded R2 stream URL", async () => {
  const meta = makeMeta("cartoon-aziz:sally-2026");
  ...
  assert.deepEqual(makeMeta("cartoon-aziz:sally-2026", [1, 48]).meta.videos.map((v) => v.episode), [1, 48]);
  const result = makeStreams("cartoon-aziz:sally-2026:1");
